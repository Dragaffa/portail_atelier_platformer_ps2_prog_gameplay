class Grimper extends Phaser.Scene {
    ...
    preload ()
    {
        ...
        this.load.spritesheet('climb', 'assets/climb.png', { frameWidth: 14, frameHeight: 44 });
    }
...
//CREATION DES ACTIONS DU PERSONNAGE
        this.player=player = this.physics.add.sprite(100, 450, 'gerard');
...
//CREATION DES ANIMATIONS DU PERSONNAGE GRACE AU SPRITESHEET
        ...
        this.anims.create({
            key: 'climb',
            frames: this.anims.generateFrameNumbers('climb', { start: 0, end: 3 }),//CE SONT LES IMAGES 5/6/7/8 QUI SONT JOUEES
            frameRate: 10,//NOMBRE D'IMAGES JOUEES
            repeat: 1//REPETITION INFINIE
        });
    ...
    /**
     * Dit si on touche une lianne ou pas
     * @returns {boolean}
     */

    update ()
    {
        if(this.player.onlianne===true){
            this.player.onlianne=false
            player.anims.play('climb');
            console.log('oui',player.onlianne)
        ...
        if(this.onLianne){
            ...
            else if (!this.downDown && !this.upDown){
